...
def stats():
    try:
        return jsonify({
            'total_examples': training_collection.count_documents({}) if training_collection else 0,
            'files_uploaded': files_collection.count_documents({}) if files_collection else 0,
            'exceptions': exception_collection.count_documents({}) if exception_collection else 0,

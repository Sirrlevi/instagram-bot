...
try:
    ...
    training_collection = db['training_data'] if db else None
    files_collection = db['uploaded_files'] if db else None
    exception_collection = db['exception_list'] if db else None
    processed_messages = db['processed_messages'] if db else None
...
def stats():
    try:
        return jsonify({
            'total_examples': training_collection.count_documents({}) if training_collection else len(bot.training_data),
            'files_uploaded': files_collection.count_documents({}) if files_collection else 0,
            'exceptions': exception_collection.count_documents({}) if exception_collection else len(bot.exception_list),
            'messages_processed': processed_messages.count_documents({}) if processed_messages else 0,
            'status': 'active' if db else 'database_error'
        })
    except Exception as e:

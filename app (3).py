# app.py - Instagram Bot - FINAL WORKING VERSION
...
class AdvancedReplyBot:
    def __init__(self):
        ...
        self.slang_groups = {
            'greetings': ['hey', 'hi', 'hello', 'sup', 'wassup', 'yo'],
            'compliments': ['cutie', 'hot', 'sexy', 'gorgeous', 'beautiful'],
            'questions': ['wyd', 'what', 'where', 'when', 'why', 'how'],
            'abuse': ['fuck', 'shit', 'bitch', 'ass', 'bc', 'mc'],
            'requests': ['send', 'give', 'show', 'share', 'dm'],
        }
    ...
    def find_best_matches(self, incoming_message, top_n=3):
        ...
        for item in self.training_data:
            ...
            category_boost = 0.2 if any(cat in self.detect_slang_category(instruction) for cat in incoming_categories) else 0
    ...
    def mix_responses(self, matches):
        ...
        for match in matches[:2]:
            ...
            parts = re.split(r'[ðŸ’€ðŸ¥€â˜ ï¸ðŸ˜­ðŸ¤£ðŸ’”]', response)
            if parts:
                response_parts.extend([p.strip() for p in parts if p.strip()])
        if response_parts:
            selected = random.sample(response_parts, min(2, len(response_parts)))
            emojis = ['ðŸ’€', 'ðŸ¥€', 'â˜ ï¸', 'ðŸ˜­', 'ðŸ¤£', 'ðŸ’”']
            emoji = random.choice(emojis)
            return f"{' '.join(selected)} {emoji}"
        return self.get_default_response()

    def get_default_response(self):
        defaults = ["sybau wtf u want ðŸ¥€", "stfu ngga busy â˜ ï¸", "leave me alone ðŸ’”", "teri maa ki chut ðŸ’€", "bhag bsdk ðŸ¤£", "die ngga â˜ ï¸"]
    ...
    def get_response(self, incoming_message, username=None, is_group=False):
        if not incoming_message or len(incoming_message.strip()) == 0:
            ...
            response = self.mix_responses(matches) if matches and matches[0]['score'] > 0.3 else self.get_default_response()
...
def upload_file():
    ...
    try:
        ...
        if result.get('success'):
            return jsonify({'success': True, 'message': f"Added {result['added']} examples", 'stats': result})
        else:
...
def test():
    ...
    if username and bot.is_user_excepted(username):
        return jsonify({'message': message, 'response': '[EXCEPTED - NO REPLY]', 'excepted': True})
...
def stats():
    return jsonify({
        'total_examples': training_collection.count_documents({}),
        'files_uploaded': files_collection.count_documents({}),
        'exceptions': exception_collection.count_documents({}),
        'messages_processed': processed_messages.count_documents({}),
        'status': 'active'
    })
...
def reload_data():
    ...
    return jsonify({'success': True, 'training': len(bot.training_data), 'exceptions': len(bot.exception_list)})
